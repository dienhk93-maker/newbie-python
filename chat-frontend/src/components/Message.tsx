  ...
  return (
    <div className={`flex gap-3 py-3 px-2 group ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`flex flex-col gap-2 ${isUser ? 'items-end max-w-[80%]' : 'items-start w-full min-w-0'}`}>
        ...
        {/* Statuses (only visible while processing/streaming) */}
        {!isUser && message.isStreaming && message.statuses && message.statuses.length > 0 && (
          <div className="flex flex-col gap-1 pl-1" style={{ marginBottom: (message.content || message.isStreaming) ? '4px' : '0' }}>
            {message.statuses.map((status, idx) => (
              <div key={idx} className="flex items-center gap-2 text-xs text-indigo-300">
                {idx === message.statuses!.length - 1 ? (
                ...
                <span className="italic opacity-80">{status}</span>
              </div>
            ))}
          </div>
        )}
